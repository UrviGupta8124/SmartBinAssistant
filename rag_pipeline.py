...
# ✅ FIX 1: Safe file path
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
file_path = os.path.join(BASE_DIR, "sample_docs", "smart_waste.txt")
...
# 3. Load embedding model (loaded ONCE)
embedding_model = HuggingFaceEmbeddings(
    model_name="ibm-granite/granite-embedding-30m-english"
)

# 4. Create FAISS vector DB (ONCE)
vector_db = FAISS.from_documents(texts, embedding_model)
...
def run_query(query: str, chat_history: list = []) -> str:
    try:
        ...
        # ✅ FIX 3: use invoke instead of run
